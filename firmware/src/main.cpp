...
#ifdef DB_SERIAL_PORT
  if (dbSerial != arSerial)
    dbSerial->begin(DB_SERIAL_BAUD);
#endif
...
uint8_t serialIn_h()
{
  ...
  while (arSerial->available() && bufferStatus == 0)
...
uint8_t parseInput(char c)
{
  ...
  {
    if (isVerb)
      arSerial->print(c); // Humans like to see what they are typing...
    ...
    case LF:
      ...
      else
      {
        ...
        {
          if (isVerb)
            arSerial->println(); // Move to new line
#ifdef DEBUG1
          dbSerial->print(F("parseInput: Received "));
          dbSerial->println(pBuf);
#endif
  ...
  if (pbPtr >= PBSIZE)
  {
    ...
    { // Buffer contains data and is full, so process the buffer (send data via GPIB)
      dataBufferFull = true;
      r = 2;
    }
  }
...
#ifdef DEBUG1
  dbSerial->print(F("sendToInstrument: Received for sending: "));
...
void execCmd(char *buffr, uint8_t dsize)
{
...
#ifdef DEBUG1
  dbSerial->print(F("execCmd: Command received: "));
  printHex(line, dsize);
#endif
  ...
  line[dsize - 1] = '\0';
#ifdef DEBUG1
  dbSerial->print(F("execCmd: Sent to the command processor: "));
  printHex(line, dsize - 2);
#endif
  // Execute the command
  if (isVerb)
    arSerial->println(); // Shift output to next line
  getCmd(line);
...
void getCmd(char *buffr)
{
  ...
  dbSerial->print(buffr);
  dbSerial->print(F(" - length:"));
  dbSerial->println(strlen(buffr));
#endif
...
#ifdef DEBUG1
  dbSerial->print("getCmd: process token: ");
  dbSerial->println(token);
#endif
  ...
  if (i < casize)
  {
...
#ifdef DEBUG1
    dbSerial->print("getCmd: found handler for: ");
    dbSerial->println(cmdHidx[i].token);
#endif
    ...
    if (cmdHidx[i].opmode & AR488.cmode)
    {
      ...
      {
#ifdef DEBUG1
        dbSerial->print(F("Calling handler with parameters: "));
        dbSerial->println(params);
#endif
    ...
    else
    {
      errBadCmd();
      if (isVerb)
        arSerial->println(F("Command not available in this mode."));
    }
  }
  else
  {
...
void addr_h(char *params)
{
  ...
  if (params != NULL)
  {
    ...
    if (val == AR488.caddr)
    {
      errBadCmd();
      if (isVerb)
        arSerial->println(F("That is my address! Address of a remote device is required."));
      return;
    }
    AR488.paddr = val;
    if (isVerb)
...
void read_h(char *params)
{
  ...
  if (params != NULL)
  {
    ...
    {
      if (isVerb)
        arSerial->println(F("Invalid termination character - ignored!"));
      void addr_h(char *params);
    }
    else if (strncmp(params, "eoi", 3) == 0)
    { // Read with eoi detection
      rEoi = true;
    }
    else
    { // Assume ASCII character given and convert to an 8 bit byte
      rEbt = true;
      eByte = atoi(params);
    }
  }
...
void clr_h()
{
  ...
  {
    if (isVerb)
      arSerial->println(F("Failed to address device"));
    return;
  }
  if (gpibSendCmd(GC_SDC))
  {
    if (isVerb)
      arSerial->println(F("Failed to send SDC"));
    return;
  }
  if (uaddrDev())
  {
    if (isVerb)
      arSerial->println(F("Failed to untalk GPIB bus"));
    return;
  }
  ...
  if (digitalRead(REN) == LOW)
  {
    ...
    if (params != NULL)
    {
      if (0 == strncmp(params, "all", 3))
      {
        ...
        {
          if (isVerb)
            arSerial->println(F("Failed to send universal LLO."));
        }
      }
    }
    else
    {
      ...
      {
        if (isVerb)
          arSerial->println(F("Failed to address the device."));
        return;
      }
      ...
      {
        if (isVerb)
          arSerial->println(F("Failed to send LLO to device"));
        return;
      }
      // Unlisten bus
      if (uaddrDev())
      {
        if (isVerb)
          arSerial->println(F("Failed to unlisten the GPIB bus"));
        return;
      }
    }
  }
...
void loc_h(char *params)
{
  ...
  if (digitalRead(REN) == LOW)
  {
    ...
    else
    {
      ...
      {
        if (isVerb)
          arSerial->println(F("Failed sending LOC."));
        return;
      }
      // Unlisten bus
      if (uaddrDev())
      {
        if (isVerb)
          arSerial->println(F("Failed to unlisten GPIB bus."));
        return;
      }
      // Set GPIB controls back to idle state
      setGpibControls(CIDS);
...
void ifc_h()
{
  if (AR488.cmode == 2)
  {
    ...
    if (isVerb)
      arSerial->println(F("IFC signal asserted for 150 microseconds"));
  }
}
...
void trg_h(char *params)
{
  ...
  if (cnt > 0)
  {
    for (int i = 0; i < cnt; i++)
    {
      ...
      {
        if (isVerb)
          arSerial->println(F("Failed to trigger device"));
        return;
      }
      // Unaddress device
      if (uaddrDev())
    ...
    if (isVerb)
      arSerial->println(F("Group trigger completed."));
  }
}
...
void spoll_h(char *params)
{
  ...
  else
  {
    ...
    while (j < 15)
    {
      ...
      if (strncmp(param, "all", 3) == 0)
      {
        ...
        j = 30;
        if (isVerb)
          arSerial->println(F("Serial poll of all devices requested..."));
        break;
      ...
      else
      {
        errBadCmd();
        if (isVerb)
          arSerial->println(F("Invalid parameter"));
        return;
      }
    }
  }
  ...
  for (int i = 0; i < j; i++)
  {
    ...
    if (val != AR488.caddr)
    {
      ...
      {
        if (isVerb)
          arSerial->println(F("Failed to retrieve status byte"));
      }
    }
  }
  if (all)
  ...
  if (isVerb)
    arSerial->println(F("Serial poll completed."));
}
...
void stat_h(char *params)
{
  ...
  if (params != NULL)
  {
    ...
    if (val & 0x40)
    {
      setSrqSig();
      if (isVerb)
        arSerial->println(F("SRQ asserted."));
    }
    else
    {
      clrSrqSig();
      if (isVerb)
        arSerial->println(F("SRQ un-asserted."));
    }
  }
  else
  {
...
void save_h()
{
  ...
  epWriteData(AR488.db, AR_CFG_SIZE);
  if (isVerb)
    arSerial->println(F("Settings saved."));
#else
...
void dcl_h()
{
  ...
  {
    if (isVerb)
      arSerial->println(F("Sending DCL failed"));
    return;
  }
  // Set GPIB controls back to idle state
  setGpibControls(CIDS);
...
void eor_h(char *params)
{
  ...
  {
    if (AR488.eor > 7)
      AR488.eor = 0; // Needed to reset FF read from EEPROM after FW upgrade
...
void ppoll_h()
{
  ...
  if (isVerb)
    arSerial->println(F("Parallel poll completed."));
}
...
void srqa_h(char *params)
{
  ...
  if (params != NULL)
  {
    ...
    if (isVerb)
      arSerial->println(isSrqa ? "SRQ auto ON" : "SRQ auto OFF");
  }
  else
  {
...
void repeat_h(char *params)
{
  ...
  if (params != NULL)
  {
    ...
    else
    {
      errBadCmd();
      if (isVerb)
        arSerial->println(F("Missing parameter"));
      return;
    }
  }
  else
  {
    errBadCmd();
    if (isVerb)
      arSerial->println(F("Missing parameters"));
  }
}
  ...
  if (params != NULL)
  {
    ...
    if (dlen)
    {
      ...
      if (strncmp(keyword, "name", 4) == 0)
      {
        ...
        {
          if (isVerb)
            arSerial->println(F("Length of name must not exceed 15 characters!"));
          errBadCmd();
        }
        return;
      }
      if (strncmp(keyword, "serial", 6) == 0)
      {
        ...
        {
          if (isVerb)
            arSerial->println(F("Serial number must not exceed 9 characters!"));
          errBadCmd();
        }
        return;
      }
...
void idn_h(char *params)
{
  ...
  if (params != NULL)
  {
    ...
    if (isVerb)
    {
      ...
      if (val == 2)
        arSerial->print(F(" with serial number"));
      arSerial->println();
    };
  }
  else
...
#ifdef DEBUG5
        if (!spe)
          dbSerial->println(F("attnRequired: Controller wants me to send data >>>"));
#endif
      }
...
void sdc_h()
{
  ...
  if (isVerb)
    arSerial->println(F("Resetting..."));
#ifdef DEBUG5
  dbSerial->print(F("Reset adressed to me: "));
  dbSerial->println(aTl);
#endif
  if (aTl)
    rst_h();
  if (isVerb)
...
{
  if (isVerb)
    arSerial->println(F("<- serial poll request ended."));
}
...
{
  if (isVerb)
    arSerial->println(F("Serial poll request received from controller ->"));
  gpibSendStatus();
  if (isVerb)
    arSerial->println(F("Status sent."));
  // Clear the SRQ bit
  AR488.stat = AR488.stat & ~0x40;
  // Clear the SRQ signal
  clrSrqSig();
  if (isVerb)
    arSerial->println(F("SRQ bit cleared (if set)."));
}
  ...
  { // Controller mode
    if (!err)
    {
      if (deviceAddressing)
      {
        ...
        {
          if (isVerb)
            arSerial->println(F("gpibSendData: Failed to unlisten bus"));
        }
...
bool gpibReceiveData()
{
  ...
  if (AR488.eor == 7)
    rEoi = true; // Using EOI as terminator
  ...
  {
    if (isVerb)
      arSerial->println(F("EOI detected!"));
    // If eot_enabled then add EOT character
    if (AR488.eot_en)
      arSerial->print(AR488.eot_ch);
  }
  ...
  {
    if (isVerb && r == 1)
      arSerial->println(F("Timeout waiting for sender!"));
    if (isVerb && r == 2)
      arSerial->println(F("Timeout waiting for transfer to complete!"));
  }
  ...
  if (AR488.cmode == 2)
  {
    ...
    {
      if (isVerb)
        arSerial->print(F("gpibSendData: Failed to untalk bus"));
    }
...
uint8_t gpibReadByte(uint8_t *db, bool *eoi)
{
  ...
  *eoi = false;
  ...
  {
    if (isVerb)
      arSerial->println(F("gpibReadByte: timeout waiting for DAV to go LOW"));
    setGpibState(0b00000000, 0b00000100, 0);
  ...
  *db = readGpibDbus();
  ...
  {
    if (isVerb)
      arSerial->println(F("gpibReadByte: timeout waiting DAV to go HIGH"));
    return 2;
  }
...
bool gpibWriteByteHandshake(uint8_t db)
{
  ...
  {
    if (isVerb)
      arSerial->println(F("gpibWriteByte: timeout waiting for receiver attention [NDAC asserted]"));
    return true;
  }
  ...
  {
    if (isVerb)
      arSerial->println(F("gpibWriteByte: timeout waiting for receiver ready - [NRFD unasserted]"));
    return true;
  }
  ...
  {
    if (isVerb)
      arSerial->println(F("gpibWriteByte: timeout waiting for data to be accepted - [NRFD asserted]"));
    return true;
  }
  ...
  {
    if (isVerb)
      arSerial->println(F("gpibWriteByte: timeout waiting for data accepted signal - [NDAC unasserted]"));
    return true;
  }
...
bool addrDev(uint8_t addr, bool dir)
{
  ...
  else
  {
    ...
    if (gpibSendCmd(GC_LAD + addr))
      return ERR;
    if (gpibSendCmd(GC_TAD + AR488.caddr))
      return ERR;
  }
  return OK;
}
